...
function BatchReporterEs(es_host, es_index, subject, es_log_level, es_timeout) {
...
BatchReporterEs.prototype._logProgress = function(newStatus, id, msg) {
    ...
    var status = this.es.index({index: this.es_index,
        type: E.TYPE.STATUS,
    ...
    var queue = this.es.index({index: this.es_index,
        type: E.TYPE.QUEUE,
        body: {
            timestamp: new Date(),
            subject: this.subject,
    ...
    return Promise.all([status, queue]);
};
